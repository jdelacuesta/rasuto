...
class OAuthHandler: NSObject, ObservableObject {
    ...
    private func getClientCredentialsToken(for service: String) async throws -> String {
        ...
        // Create auth header
        let credentialString = "\(config.clientID):\(config.clientSecret)"
        print("🔍 DEBUG - Raw credential string: \(credentialString)")
        
        let credentials = credentialString
            .data(using: .utf8)!
            .base64EncodedString()
        
        print("🔍 DEBUG - Base64 credentials: \(credentials)")
        ...
        // Set request body - URL encode the scope
        let encodedScope = config.scope.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? config.scope
        ...
        do {
            ...
            print("📘 Request Headers: \(request.allHTTPHeaderFields?.filter { $0.key != "Authorization" } ?? [:])")
            print("📘 Request Body: \(body)")
            print("🔍 DEBUG - Credentials being used:")
            print("🔍 ClientID: \(config.clientID)")
            print("🔍 ClientSecret (first 5 chars): \(config.clientSecret.prefix(5))...")
            print("🔍 Token Endpoint: \(config.tokenEndpoint)")
            print("🔍 Auth Header: Basic \(credentials.prefix(10))...")
