...
class EbayNotificationManager: ObservableObject {
    ...
    init(ebayService: EbayAPIService) {
        ...
        // As a fallback, also start the polling refresh timer
        startRefreshTimer()
    }
    ...
    // MARK: - Refresh & Update
        ...
        private func startRefreshTimer() {
            ...
            // This is used as a fallback in case push notifications fail
            refreshTask = Task {
        ...
        func untrackItem(id: String) async throws -> Bool {
            // Untrack with eBay API
            do {
                let success = try await ebayService.untrackItem(id: id)
                
                if success {
                    // Remove from locally tracked items
                    let defaults = UserDefaults.standard
                    var trackedItems = defaults.stringArray(forKey: "ebay_tracked_items") ?? []
    ...
    extension EbayNotificationManager {
        ...
        func addMockAlerts() {
            ...
            // Price drop alert
            let priceDropAlert = NotificationAlert(
            ...
            // Add another price drop alert for the coffee maker
            let coffeeMakerAlert = NotificationAlert(
            ...
            let auctionEndingAlert = NotificationAlert(
                ...
                isRead: false,
                auctionEndTime: Date().addingTimeInterval(2700), // 45 minutes from now
                currentBid: 1850.00
            )
            
            // Item sold alert
            let itemSoldAlert = NotificationAlert(
            ...
            // Back in stock alert
            let backInStockAlert = NotificationAlert(
            ...
            // Add all mock alerts to the alerts array
            alerts.append(priceDropAlert)
            alerts.append(coffeeMakerAlert)
            alerts.append(auctionEndingAlert)
            alerts.append(itemSoldAlert)
            alerts.append(backInStockAlert)
        }
    }
