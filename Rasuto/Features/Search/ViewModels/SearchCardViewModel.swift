...
class SearchCardViewModel: ObservableObject {
    ...
    private func setupAPIServices() {
        // Use SerpAPI + fallback architecture
        do {
            let apiConfig = APIConfig()
            
            // SerpAPI services for primary search
            ...
            // Axesso for Amazon fallback
            let axessoAmazon = try apiConfig.createAxessoAmazonService()
            
            // Oxylabs for universal fallback
            let oxylabsService = try apiConfig.createOxylabsService()
            
            apiServices = [serpGoogleShopping, serpEbay, serpWalmart, axessoAmazon, oxylabsService]
        } catch {
    ...
    private func performSearch(query: String) async {
        ...
        if !allResults.isEmpty {
            await persistentCache.addProducts(allResults)
            print("📱 SearchCardViewModel: Added \(allResults.count) products to persistent cache")
        }
    }
